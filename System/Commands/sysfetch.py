...
def get_battery() -> str:
    if os.name == "nt":
        try:
            result = subprocess.check_output("wmic path win32_battery get estimatedchargeleft", shell=True, text=True, stderr=subprocess.STDOUT)
            for line in result.splitlines():
                line = line.strip()
...
def get_packages() -> str:
    try:
        programs_dir = ROOT / "CoreOS" / "Programs"
...
ram_total, ram_free = get_memory()
info_lines.append(f"Memory: {ram_free}/{ram_total}")
